...
- feat: optional biometric-only user-authentication gating for generated keys, via a new optional `KeyAuthenticationPolicy` argument of `generate` (no device PIN/passcode fallback, so `invalidateOnEnrollmentChange` cannot be bypassed). Additive and backward compatible: `generate(keyTag)` behavior is unchanged. Adds the `USER_CANCELED`, `USER_NOT_AUTHENTICATED`, `AUTH_FAILED`, `BIOMETRICS_NOT_AVAILABLE` and `PASSCODE_NOT_SET` error codes.
