...
- feat: optional user-authentication gating (biometric with device PIN/passcode fallback) for generated keys, via a new optional `KeyAuthenticationPolicy` argument of `generate`. Additive and backward compatible: `generate(keyTag)` behavior is unchanged. Adds the `USER_CANCELED`, `USER_NOT_AUTHENTICATED`, `AUTH_FAILED`, `BIOMETRICS_NOT_AVAILABLE` and `PASSCODE_NOT_SET` error codes.
